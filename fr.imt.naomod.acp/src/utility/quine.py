...
def remove_redundant_list(liste):
    new_list = []
    for i in liste:
...
### convert a string unchecked into a binary with don'tcare
def convert(stri):
    res = []
    for car in stri:
        if (car=='-'):
...
# with input a list of binary
#main function
#def main():
def quine(n_var, listbin):
...
#     #make a group list
    ...
    for i in range(0, len(listbin)):
        a[i] = ''.join(str(e) for e in listbin[i])
        #count the num of 1
        index = a[i].count('1')
    ...
    for prime in primes:
        ...
        for i in range(len(unchecked)):
            for j in prime:
                if j == i:
                    #s= s+binary_to_letter(unchecked[i])+' + '
                    # print ("convert " + str(convert(unchecked[i])))
                    res.append(convert(unchecked[i]))
        #print (s[:(len(s)-3)])
        return res
# --- end of quine
